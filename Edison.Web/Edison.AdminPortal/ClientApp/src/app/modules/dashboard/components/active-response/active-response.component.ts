...
export class ActiveResponseComponent implements OnInit, OnChanges {
  ...
  updateResponseStatus() {
    if (
      ...
      this.actionPlanSuccessful = !this.activeResponse.actionPlan.openActions.some(
        action => action.status !== ActionStatus.Success
      );
      this.actionPlanNeedsLocation =
        this.responseNeedsLocation &&
        this.activeResponse.actionPlan.openActions.some(
          action => action.status === ActionStatus.Skipped
        );
      this.actionPlanHasErrors = this.activeResponse.actionPlan.openActions.some(
        action =>
          action.status === ActionStatus.Error ||
          action.status === ActionStatus.Unknown
      );
      this.actionPlanPartialSuccess = this.activeResponse.actionPlan.openActions.some(
        action => action.status === ActionStatus.Success
      );
    } else {
  ...
  setLocation() {
    ...
    this.store.dispatch(
      new SelectActiveResponse({ response: this.activeResponse })
    );
    this.store.dispatch(
