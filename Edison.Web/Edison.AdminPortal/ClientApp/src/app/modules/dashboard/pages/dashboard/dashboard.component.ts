...
import {
    GetActionPlan, GetActionPlans, SelectActionPlan
} from '../../../../reducers/action-plan/action-plan.actions';
import { ActionPlan, AddEditAction } from '../../../../reducers/action-plan/action-plan.model';
import {
    actionPlansSelector, selectedActionPlanSelector
} from '../../../../reducers/action-plan/action-plan.selectors';
import { AppActionTypes, AppPage, SetPageData, ToggleOverlay } from '../../../../reducers/app/app.actions';
import {
    AddChat, ChatActionTypes, EndConversation, GetChatAuthToken, SendNewMessage,
    SendUserReadReceipt, ToggleAllUsersChatWindow, ToggleUserChatWindow, UpdateUserReadReceipt
} from '../../../../reducers/chat/chat.actions';
import { Message } from '../../../../reducers/chat/chat.model';
import {
    chatActiveMessagesSelector, chatActiveUsersCountSelector, chatActiveUserSelector,
    chatAllMessagesSelector, chatAuthSelector
...
import {
    EventActionTypes, GetEvents, SelectActiveEvent, ShowEvents, UpdateEvent, ShowEventInEventBar
} from '../../../../reducers/event/event.actions';
import { Event, EventInstance, EventType } from '../../../../reducers/event/event.model';
import {
    activeMobileEventsSelector, eventsSelector
...
import {
    activeResponsesSelector, responsesExist, responsesSelector, showSelectingLocationSelector, activeResponseSelector
...
export class DashboardComponent implements OnInit, OnDestroy {
    ...
    deactivateResponse(responseId: string) {
        this.store.dispatch(new CloseResponse({ responseId, state: ResponseState.Inactive }));
    }
    ...
    updateResponseLocation(payload: { geolocation: GeoLocation, responseId: string}) {
    ...
    protected updateResponseActions(payload: { response: Response, actions: AddEditAction[], isCloseAction: boolean }) {
        this.store.dispatch(new UpdateResponseActions(payload));
    }

    protected postNewResponse({ event, actionPlan }: { event: Event, actionPlan: ActionPlan }) {
    ...
    protected sendNewMessage(messageContent: { message: string, userId: string }) {
    ...
    protected updatePins(devices: Device[], events: Event[], responses: Response[]) {
        // Device pins
        ...
        const eventPins: MapPin[] = events
            ...
            .map(event => {
                return ({
                    ...event.device,
                    event: event,
                    color: getEventColor(event, this.getActiveResponse(responses, event.eventClusterId))
                });
            })
    ...
    private _initMobileEventsData() {
        this.activeMobileEvents$ = this.store
            ...
            .subscribe(events => {
                this.activeEvent = events.find(event => event.events.some(ee => ee.metadata.userId === this.userId));
    ...
    private _selectActiveEventInstance() {
        if (this.activeEvent) {
            this.activeEventInstance = this.activeEvent.events
    ...
    private _initMessageData() {
        this.messagesSub$ = this.store
            ...
            .subscribe(messages => {
                if (!this.activeMessages || this.activeMessages.length < messages.length) {
    ...
    private _initData() {
        this.activeUser$ = this.store
            .pipe(select(chatActiveUserSelector), filter(user => (user && user.userId !== null && user.name !== null && this.userId !== user.userId)))
            .subscribe(user => {
        ...
        this.actionPlans$ = this.store.pipe(select(actionPlansSelector)).subscribe(actionPlans => this.actionPlans = actionPlans);

        this.showSelectingLocation$ = this.store.pipe(select(showSelectingLocationSelector)).subscribe(state => {
    ...
    private _initObservables() {
        ...
        this.selectedActionPlan$ = this.store.pipe(select(selectedActionPlanSelector));
