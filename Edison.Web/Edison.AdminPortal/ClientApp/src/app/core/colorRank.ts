...
export const getRankingColor = (colors: string[]) => {
    ...
    if (hasRed) { return SpinnerColor.Red; }

    const hasYellow = colorsLowered.some(c => c === 'yellow');
    if (hasYellow) { return SpinnerColor.Yellow; }

    const hasBlue = colorsLowered.some(c => c === 'blue');
    if (hasBlue) { return SpinnerColor.Blue; }

    const hasGreen = colorsLowered.some(c => c === 'green');
    if (hasGreen) { return SpinnerColor.Green; }

    const hasGrey = colorsLowered.some(c => c === 'grey');
    if (hasGrey) { return SpinnerColor.Grey; }
};

export const getEventColor = (event: Event, response: Response): SpinnerColor | string => {
    if (response) {
        // response is active
        ...
        if (expired) { return SpinnerColor.Grey; } // default for a timed out response
